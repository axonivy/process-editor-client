...
test.describe('viewport bar', () => {
  ...
  test('scroll vertical', async ({ page }) => {
    await scrollAndAssert(page, { x: 0, y: 100 }, 'scale(1) translate(0,-48)', { x: 0, y: -100 });
    await scrollAndAssert(page, { x: 0, y: -200 }, 'scale(1) translate(0,152)', { x: 0, y: 100 });
    await scrollAndAssert(page, { x: 0, y: 100 }, ORIGIN_VIEWPORT, { x: 0, y: 0 });
  });

  test('scroll horizontal', async ({ page }) => {
    await scrollAndAssert(page, { x: 100, y: 0 }, 'scale(1) translate(-100,52)', { x: -100, y: 0 });
    await scrollAndAssert(page, { x: -200, y: 0 }, 'scale(1) translate(100,52)', { x: 100, y: 0 });
    await scrollAndAssert(page, { x: 100, y: 0 }, ORIGIN_VIEWPORT, { x: 0, y: 0 });
  });

  test('scroll horizontal (shift)', async ({ page }) => {
    await page.keyboard.down('Shift');
    await scrollAndAssert(page, { x: 0, y: 100 }, 'scale(1) translate(-100,52)', { x: -100, y: 0 });
    await scrollAndAssert(page, { x: 0, y: -200 }, 'scale(1) translate(100,52)', { x: 100, y: 0 });
    await scrollAndAssert(page, { x: 0, y: 100 }, ORIGIN_VIEWPORT, { x: 0, y: 0 });
