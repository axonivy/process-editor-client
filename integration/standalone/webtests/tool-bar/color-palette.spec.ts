...
test.describe('tool bar - color palette', () => {
  ...
  test('colorize node', async ({ page }) => {
    ...
    await colorizeElement(page, [startElement]);
  ...
  test('colorize connector', async ({ page }) => {
    ...
    await colorizeElement(page, [connector]);
  ...
  test('colorize lane', async ({ page }) => {
    ...
    await colorizeElement(page, [lane]);
  ...
  test('colorize multiple elements', async ({ page, browserName }) => {
    ...
    await colorizeElement(page, [startElement, endElement]);
    await expect(startElement).toHaveAttribute('style', 'stroke: rgb(0, 0, 255);');
    await expect(endElement).toHaveAttribute('style', 'stroke: rgb(0, 0, 255);');
  });

  async function colorizeElement(page: Page, elements: Locator[]): Promise<void> {
    ...
    await multiSelect(page, elements);
    await expect(dynamicTools).toBeVisible();
    await expect(colorPaletteBtn).toBeVisible();
    await colorPaletteBtn.click();
